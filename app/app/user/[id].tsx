...
import { ChevronLeft, MoreHorizontal, MapPin, Grid3x3, Bookmark } from 'lucide-react-native';
...
export default function UserProfileScreen() {
  ...
  const { data, isLoading, isError } = useProfile(id, { autoCreate: false });
    ...
    <View className="flex-1 bg-kara-bg">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View style={{ position: 'relative', height: 230 }}>
          ...
          />
          {/* Bouton back */}
          <View style={{ position: 'absolute', top: insets.top + 8, left: 14, zIndex: 4 }}>
            <Pressable
              onPress={() => router.back()}
              style={{ width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(17,17,24,0.72)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', alignItems: 'center', justifyContent: 'center' }}
            >
        ...
        {/* Onglet Favoris — état vide statique */}
        {tab === 'favs' && (
          <View style={{ alignItems: 'center', paddingTop: 40 }}>
            <Text style={{ color: '#9594B5', fontFamily: 'Inter_400Regular', fontSize: 14 }}>
              Les favoris de @{username} apparaîtront ici
            </Text>
          </View>
        )}
      </ScrollView>
